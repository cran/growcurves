...
##
## Load education dataset with one treatment arm
## and multivariate teacher effects
##
data(dateduc)
...
mod <- function(x, niter, nburn, nthin){
		 dpgrowmm(y = dateduc$y, subject = dateduc$subject, trt = dateduc$trt, time = dateduc$time, 
						n.random = 3, n.fix_degree = 2,  subj.aff = dateduc$subj.aff, 
						W.subj.aff = dateduc$W.subj.aff, multi = TRUE, n.iter = niter, n.burn = nburn,
...
test_that("mmi option of dpgrowmm returns expected objects under multivariate MM effects", {
	...
	parms		<- samples(MMI)
	pr		<- MMI$plot.results
	num.subj	<- length(unique(dateduc$subject))
	nrandom		<- ncol(srm$Z)
	nsessions	<- ncol(dateduc$W.subj.aff)
	...
	expect_that(ncol(srm$X),equals(2))
	...
	expect_that(length(residuals(MMI)),equals(length(dateduc$y)))
