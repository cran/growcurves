...
#' for a focus (or all) multiple membership term(s) may be compared under different model formulations (one vs. multiple terms) and 
#' for differing prior formulations (e.g. "mmcar" vs. "mmdp").
#'
#' @param objects A list object where each entry is either a \code{dpgrowmm} or \code{dgrowmult} object
...
#'		   The entries are term names for \code{dpgrowmult} objects and group names for \code{dpgrowmm} objects.  
...
#'	\item{p.all}{A \code{ggplot2} only rendered where all objects are of class \code{dpgrowmult} and have common number of terms and effects.  Plot compares effects across objects.}
...
#' @seealso \code{\link{dpgrowmm}}, \code{\link{dpgrowmult}},
#' @author Terrance Savitsky \email{tds151@@gmail.com}
#' @aliases effectsplot
#' @export
effectsplot <- function(objects, mm.terms = NULL, prior.labs = NULL, center = TRUE, axis.labs = NULL, group.plot = NULL, smoother = FALSE)
{
  ## check inputs
  ...
  if( !is.null(group.plot) & is.null(mm.terms) ) warning("\n The 'groups.plot' input will be ignored when not employing 'mm.terms'.")
  ...
  if( !(is.null(mm.terms))  )
  {
	...
	## memo: mm.terms is either a numeric vector of term positions or a character vector of term names
  	...
  	for( i in 1:nty)
  	{
		if( class(objects[[i]]) == "dpgrowmult" )
		{
			...
			us.summary[[i]]		= all.us.i[all.groups.i == mm.nterms[i],]
		}
  	}
	...
	## check consistency of inputs
  	...
  	sessions		= 1:nrow(us.summary[[1]])
  	if( !is.null(group.plot) ) 
  	{
		if( length(group.plot) != length(us.summary[[1]]) ) stop("\nLength of 'group.plot' must be equal to number of effects for each term.")
  		group 			= group.plot
  	}else{
		group			= groups[[1]]
  	}
  	...
  	for( i in 1:nty )
  	{
  		dat[[i]] 			= data.frame(sessions,group,us.summary[[i]],prior.labs[i])
  		names(dat[[i]])		= c("session","groups","low","mean","high","prior") ## use name 'groups' to not confuse ggplot group factor
		if( center == TRUE )
		{
			dat[[i]]			= split(dat[[i]],dat[[i]]$group) ## group mean center
  	...
  	p.term				= ggplot(data=dat.term,aes(x=session, y = mean, shape = prior, colour = prior))
  	l				= geom_point()
  	l.2				= geom_smooth(aes(group=prior, colour = prior), method = "loess", span = 1.0, size = 1.1, se = FALSE)##, colour = "pink")
  	axis	 			= labs(x = axis.labs[1], y = axis.labs[2], shape = "Prior", colour = "Prior")
  	f				= facet_wrap(~groups, scales="free")
  	if( smoother == TRUE)
  	{
  		p.term			= p.term + l + l.2 + f + axis
  	}else{
		p.term			= p.term + l + f + axis
  	}
  	dev.new()
  	print(p.term)
  ...
  for( i in 1:nty)
  {
	if( class(objects[[i]]) == "dpgrowmult" )
	{
		...
		numt.block[[i]]		<- as.numeric(table(objects[[i]]$summary.results$map.grp$label.input)) ## number of sessions per group.  here we assign them to a block so they may be plotted.
	}
  }
  ...
  if( all.flag == 1 ) ## identical length treatments and dosages within treatments
  {
	...
	numt			= numt.block[[1]]
	block			= factor(ulabs) 
	nty.n			= length(numt) ## length of treatments within each object (versus number of object, nty).
	...
	for( n in 1:nty )
	{
		if( class(objects[[n]]) == "dpgrowmm" )
		{
			...
			grouping.n		<- objects[[n]]$summary.results$map.grp$label.input
			for( t in 1:nty.n ) 
			{
				posn.t			<- which(grouping.n == ulabs.block[[n]][t])
		...
		for(i in 1:nty.n)
		{
			dat[[i]]			= data.frame(prior.labs[n],1:numt[i],block[i],u.summary.n[[i]]) ## using treat to facet plot
			names(dat[[i]])		= c("prior","session","block","low","mean","high")
  ...
  session <- group <- block <- low <- mean <- high <- prior <- NULL; rm(session); rm(group); rm(low); rm(mean); rm(high); rm(prior); rm(block)
