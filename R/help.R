...
#' all \code{q} parameters receive the DP prior.  Additional sets of random effects under different (than subject)
#' grouping factors are accommodated through multiple membership constructions, each of which maps a set of effects under
...
#'
#' \code{\link{dpgrowmm}} is very similar to \code{dpgrow}, but it adds an additional set of random effects
#' which aren't grouped by subject that may be used to inject treatment dosage or attendance patterns that
#' is mapped back to clients via a multiple membership weight matrix.
#'
...
#' 
#' \code{\link{growplot}} uses model outputs from \code{dpgrow}, \code{dpgrowmm} and \code{dpgrowmult} 
...
#'
#' \code{\link{trtplot}} uses model outputs from \code{dpgrow}, \code{dpgrowmm} and \code{dpgrowmult}
